    ...
    <div>
        ...
        <app-body style="justify-content: center;">
            <div style="min-height: 85vh;">
                <div v-show="!eidtAddress">
                    <div class="user-info-container">
                        <div class="user-info-details">
                            <el-upload action="http://localhost:8088/file/" :on-success="fileHandleSuccess"
                                :file-list="imgFileList" accept="image/*">
                                <el-image style="width: 120px; height: 120px;border-radius: 10px;"
                                    :src="userInfo.avatar" fit="contain"></el-image>
                            </el-upload>
                            <div class="user-info-details-text">
                                <div class="user-info-details-text-nickname">{{ userInfo.nickname }}</div>
                                <div class="user-info-details-text-time">{{ userInfo.signInTime }}加入"idlePlanet"</div>
                                <div class="user-info-details-text-edit">
                                    <el-button type="primary" plain
                                        @click="userInfoDialogVisible = true">编辑个人信息</el-button>
                                </div>
                                <el-dialog @close="finishEdit" title="编辑个人信息" :visible.sync="userInfoDialogVisible"
                                    ...
                                    <el-input v-model="userInfo.sex" :disabled="notUserSexEdit" @change="saveUserSex">
                                        <el-button slot="append" type="warning" icon="el-icon-edit"
                                            @click="notUserSexEdit = false">编辑
                                        </el-button>
                                    </el-input>
                                    <div class="edit-tip">年龄</div>
                                    <el-input v-model="userInfo.age" :disabled="notUserAgeEdit" @change="saveUserAge">
                                        <el-button slot="append" type="warning" icon="el-icon-edit"
                                            @click="notUserAgeEdit = false">编辑
                                        </el-button>
                                    </el-input>
                                    <div class="edit-tip">手机号</div>
                                    <el-input v-model="userInfo.phone" :disabled="notUserPhoneEdit"
                                        @change="saveUserPhone">
                                        <el-button slot="append" type="warning" icon="el-icon-edit"
                                            @click="notUserPhoneEdit = false">编辑
                                        </el-button>
                                    </el-input>
                                    <div class="edit-tip">所在校区</div>
                                    <el-input v-model="userInfo.campus" :disabled="notUserCampusEdit"
                                        @change="saveUserCampus">
                                        <el-button slot="append" type="warning" icon="el-icon-edit"
                                            @click="notUserCampusEdit = false">编辑
                                        </el-button>
                                    </el-input>
                        ...
                        <div class="user-info-splace1">
                            <el-button v-if="userInfo.picture === null" type="primary" plain @click="eidma = true">
                                上传收款码</el-button>
                            <el-button v-else type="primary" plain @click="eidma = true">
                                我的收款码</el-button>
                        </div>
                    ...
                    <div class="idle-container">
                        <el-tabs v-model="activeName" @tab-click="handleClick">
                            <el-tab-pane label="我上架的" name="1"></el-tab-pane>
                            <el-tab-pane label="我下架的" name="2"></el-tab-pane>
                            <el-tab-pane label="我的收藏" name="3"></el-tab-pane>
                            <el-tab-pane label="租售记录" name="4"></el-tab-pane>
                            <el-tab-pane label="购买清单" name="5"></el-tab-pane>
                        </el-tabs>
                                    ...
                                    <div class="idle-container-list-item-text">
                                        ...
                                        <div class="idle-item-foot">
                                            <div class="idle-price">￥{{ item.idlePrice }}
                                                {{ (item.orderStatus == 1 && (activeName === '4' || activeName === '5')) ?
                                                orderStatus[item.orderStatus] : '' }}
                                                {{ (item.orderStatus == 2 && (activeName === '4' || activeName === '5')) ?
                                                orderStatus1[item.orderStatus] : '' }}
                                            </div>
                                            <el-button v-if="activeName !== '4' && activeName !== '5'" type="danger"
                                                size="mini" slot="reference" plain
                ...
                <el-dialog :visible.sync="eidma" title="收款码">
                    <el-form>
                        <el-form-item>
                            <div v-if="userInfo.picture === null"
                                style="display: flex; justify-content: center; align-items: center;">
                                <el-upload class="upload-demo" drag action="http://localhost:8088/file/"
                                    :on-success="fileHandleSuccess1" :file-list="imgFileList" accept="image/*" multiple>
                                    <i class="el-icon-upload"></i>
                                    <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
                                </el-upload>
                            </div>
                            <div v-else style="display: flex; justify-content: center; align-items: center;">
                                <el-upload action="http://localhost:8088/file/" :on-success="fileHandleSuccess1"
                                    :file-list="imgFileList" accept="image/*">
                                    <el-image style="width: 500px; height: 500px;" :src="userInfo.picture"
                    ...
                    <span slot="footer" class="dialog-footer">
                        <div style="display: flex; justify-content: center; align-items: center;">
                            <el-button @click="eidma = false">确定</el-button>
                        </div>
                    </span>
                </el-dialog>
                <div v-show="eidtAddress" class="address-container">
                    ...
                    <div class="address-container-add">
                        ...
                        <div class="address-container-add-item">
                            <span class="demonstration">学校/校区/宿舍楼</span>
...
export default {
    ...
    data() {
        return {
            ...
            notUserSexEdit: true,
            notUserAgeEdit: true,
            notUserPhoneEdit: true,
            notUserCampusEdit: true,
    ...
    methods: {
        ...
        saveUserSex() {
            this.notUserSexEdit = true;
            this.$api.updateUserPublicInfo({
                sex: this.userInfo.sex
            }).then(res => {
                console.log(res);
                this.$globalData.userInfo.sex = this.userInfo.sex;
            })

        },
        saveUserAge() {
            this.notUserAgeEdit = true;
            this.$api.updateUserPublicInfo({
                age: this.userInfo.age
            }).then(res => {
                console.log(res);
                this.$globalData.userInfo.age = this.userInfo.age;
            })

        },
        saveUserPhone() {
            this.notUserPhoneEdit = true;
            this.$api.updateUserPublicInfo({
        ...
        saveUserCampus() {
            this.notUserCampusEdit = true;
            this.$api.updateUserPublicInfo({
                campus: this.userInfo.campus
            }).then(res => {
                console.log(res);
                this.$globalData.userInfo.campus = this.userInfo.campus;
            })
        },
        savePassword() {
        ...
        handleAddressChange(value) {
            console.log(value);
            this.addressInfo.schoolName = value[0];
            this.addressInfo.campusName = value[1];
            this.addressInfo.dormitoryName = value[2];
        },
        handleEdit(index, row) {
            console.log(index, row);
            this.addressInfo = JSON.parse(JSON.stringify(row));
            this.selectedOptions = ['', '', ''];
            this.selectedOptions[0] = row.schoolName;
            this.selectedOptions[1] = row.campusName;
            this.selectedOptions[2] = row.dormitoryName;
        },
        ...
        fileHandleSuccess1(response, file, fileList) {
            console.log("file:", response, file, fileList);
            let imgUrl = response.data;
            this.imgFileList = [];
            this.$api.updateUserPublicInfo({
                ...
                console.log(res);
                this.userInfo.picture = imgUrl;
                this.$globalData.userInfo.picture = imgUrl;
            })
        },
        update(data) {
